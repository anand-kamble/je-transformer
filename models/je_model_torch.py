...
class JEModel(nn.Module):
    ...
    def forward(
        ...
        enc_proj = torch.tanh(self.enc_proj(enc_pooled))  
        ...
        enc_ctx = enc_proj * (1.0 + gamma) + beta  
        ...
        if cat_for_embed.dim() == 2:
            ...
            prev_acc_emb = F.embedding(safe_idx, cat_for_embed)  
        else:
            
            B, T = safe_idx.shape
            b = torch.arange(B, device=safe_idx.device).unsqueeze(-1).expand(B, T)  
            prev_acc_emb = cat_for_embed[b, safe_idx]  
        ...
        if cat_for_pointer.dim() == 3:
            ...
            cat_for_pointer = (
                ...
                .unsqueeze(1)                 
                .expand(B, T, -1, -1)         
                .reshape(B * T, cat_for_pointer.shape[1], cat_for_pointer.shape[2])  
            )
